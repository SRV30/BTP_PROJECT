export const DashboardHeader = ({ user }) => {
  const firstName = user?.name?.split(' ')[0] || 'Sahil'

  return (
    <header className="flex items-center justify-between gap-4">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-white sm:text-4xl">Hello, {firstName} 👋</h1>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] p-1 pr-3">
          <img
            alt={`${firstName} profile avatar`}
            className="h-10 w-10 rounded-2xl object-cover"
            src={user?.avatar || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=120&q=80'}
          />
          <span className="hidden text-sm font-semibold text-white sm:inline">{firstName}</span>
        </div>
      </div>
    </header>
  )

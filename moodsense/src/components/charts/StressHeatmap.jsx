      ...
      <div className="grid grid-cols-[44px_1fr] items-center gap-3" key={row.day}>
        ...
        <div className="grid grid-cols-7 gap-2">
          {row.values.map((value, index) => (
            <span className={`h-7 rounded-lg ${stressColors[value] || stressColors[1]}`} key={`${row.day}-${index}`} title={`${row.day} stress ${value}`} />
          ))}
        </div>
      </div>
    )) : <p className="text-sm text-slate-400">No stress heatmap data available.</p>}
